...
def main() -> None:
    ...
    with student_table_container:
        st.subheader(body="📌 View Student Details", divider=True)

        student_dataframe = df_fetch.get_student_df()
        st.dataframe(data=student_dataframe, use_container_width=True, hide_index=True)
...
def show_student_editor():
    ...
    with student_edit_container:
        ...
        selected_admission_number = st.text_input(
            label="Enter the Admission number", key="temp_admission_number"
        )
        ...
        if selected_admission_number in fetch.get_all_admission_numbers():
            (
                ...
                division,
                house,
            ) = get_name_class_division_house_from_admission_number(
                selected_admission_number
            )
            selected_name = st.text_input(label="Enter the student name", value=name)
            selected_class = st.selectbox(
                label="Select the class", options=classes, index=classes.index(class_)
            )
            ...
            st.warning(
                ...
                if selected_admission_number == "" or selected_admission_number is None
